...
const Home: NextPage = () => {
  ...
  const [agente, setAgente] = useState({} as Agente);
  ...
  const pesquisarMatriculaAgente = async (id: any) => {
    try {
      setLoading(true);
      ...
      onNotification("error", {
        message: "Erro!",
        description: "Erro ao buscar agente. Insira uma matricula válida",
      });
    }
  };
                        ...
                        <tr key={item.id}>
                          ...
                          <td>
                            {item.locado === "true" ? (
                              <Popconfirm
                                placement="topRight"
                                title={`Você confirma a entregua deste livro?`}
                                onConfirm={entrega}
                                okText="Sim"
                                cancelText="Não"
                              >
        ...
        <Spin indicator={antIcon} spinning={loading}>
          ...
          >
            ...
            {tipoUsuario === "Detento" ? (
              ...
              <FormItem name="idPessoa" label="Pesquisar Matricula">
                <Search
                  placeholder="Insira a matricula do agente"
                  onSearch={pesquisarMatriculaAgente}
                  style={{ width: "100%" }}
                />
              </FormItem>
            ) : null}
            ...
            {verAgente === true ? (
              <>
                <FormItem name="codAgente" label="">
                  <h4>Codigo: {agente.id}</h4>
                </FormItem>
                <FormItem name="nomeAgente" label="">
                  <h4>Nome: {agente.nome}</h4>
                </FormItem>
              </>
            ) : verDetento === true ? (
              <>
